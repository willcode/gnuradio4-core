 ...
 * the result, and no intermediate sample reaches memory. A fused run reaches the same members at run
 * time and can do neither. It drives one stage at a time over a scratch buffer, so every intermediate
 * sample is written and read back, and each member keeps its own call. The four arms price that
 * difference on chains of stateless float stages of a few instructions each:
 *
 *   - graph : N blocks with a ring on every edge, which is the scheduler's ordinary path,
 *   - fused : the same graph with enable_fusion, one run of N members over one scratch buffer,
 *   - merged: the N stages nested into one block by gr::Merge, in a graph of source, block and sink,
 *   - loop  : the composed arithmetic in one loop over one buffer, the bound for the other three.
 *
...
[[nodiscard]] RunStats readPlan(const auto& scheduler, const Sink& sink) {
    RunStats stats{sink._acc, sink._nReceived, 0UZ, 0UZ, 0UZ};
    for (const auto& job : scheduler.fusionPlan()) {
        stats.nRuns += job.size();
        for (const auto& run : job) {
            stats.nStages += run.stages.size();
            stats.nMembers += run.members.size();
        }
    }
    return stats;
}
...
template<typename... TStages>
[[nodiscard]] RunStats runChain(std::size_t ring, bool fusion, std::size_t nSamples = kSamples) {
    ...
    gr::scheduler::Simple<gr::scheduler::ExecutionPolicy::singleThreaded> scheduler{gr::property_map{{"enable_fusion", fusion}}};
    expect(scheduler.exchange(std::move(flow)).has_value());
    expect(scheduler.runAndWait().has_value());
    return readPlan(scheduler, sink);
}
...
[[nodiscard]] RunStats runMerged(std::size_t ring, std::size_t nSamples = kSamples) {
    ...
    gr::scheduler::Simple<gr::scheduler::ExecutionPolicy::singleThreaded> scheduler{gr::property_map{{"enable_fusion", false}}};
    expect(scheduler.exchange(std::move(flow)).has_value());
    expect(scheduler.runAndWait().has_value());
    return readPlan(scheduler, sink);
}
...
/// where one (ring, chain length) shape's four arms sit in the arm list
struct Row {
    std::size_t ring    = 0UZ;
    std::size_t nStages = 0UZ;
    std::size_t graph   = 0UZ;
    std::size_t fused   = 0UZ;
...
void appendArms(std::vector<Arm>& arms, std::vector<Row>& rows, std::size_t ring) {
    ...
    Row row{ring, n, arms.size(), arms.size() + 1UZ, arms.size() + 2UZ, arms.size() + 3UZ};
    arms.emplace_back(std::format("graph  N={} ring={:>5}", n, sizeLabel(ring)), [ring] { expect(eq(runChain<TStages...>(ring, false).nReceived, kSamples)); });
    arms.emplace_back(std::format("fused  N={} ring={:>5}", n, sizeLabel(ring)), [ring] { expect(eq(runChain<TStages...>(ring, true).nReceived, kSamples)); });
...
inline const boost::ut::suite<"merge chain"> _merge_chain = [] {
    ...
    "every arm computes the same stream and takes the path it claims"_test = [] {
        ...
        for (const std::size_t ring : kRingSizes) {
            const RunStats one   = runChain<Scale>(ring, false, kShort);
            const RunStats two   = runChain<Scale, Offset>(ring, false, kShort);
            const RunStats four  = runChain<Scale, Offset, Clip, Quantize>(ring, false, kShort);
            const RunStats fused = runChain<Scale, Offset, Clip, Quantize>(ring, true, kShort);

            expect(eq(one.nReceived, kShort));
            expect(eq(four.nReceived, kShort));
            expect(eq(runChain<Scale>(ring, true, kShort).nRuns, 0UZ)) << "a single stage is no run";
            expect(eq(runChain<Scale, Offset>(ring, true, kShort).nMembers, 2UZ));
            expect(eq(fused.nRuns, 1UZ)) << "the fused arm formed no run";
            expect(eq(fused.nStages, 1UZ)) << "four composed members are one stage";
            expect(eq(fused.nMembers, 4UZ)) << "the run holds every member of the chain";
            ...
            expect(eq(mergedFour.nRuns, 0UZ)) << "the merged block is one block and forms no run";
            ...
            expect(std::abs(four.acc - fused.acc) <= kEps * std::abs(four.acc)) << "graph and fused disagree at N=4";
    ...
    "compile-time merging against the fused run and the hand-written loop"_test = [] {
        std::vector<Arm>         arms;
        std::vector<Row>         rows;
        ...
        for (const std::size_t ring : kRingSizes) {
            ...
            arms.emplace_back(std::format("floor  graph  ring={:>5}", sizeLabel(ring)), [ring] { expect(eq(runChain<>(ring, false).nReceived, kSamples)); });
        ...
        for (std::size_t r = 0UZ; r < kRingSizes.size(); ++r) {
            ...
            for (const Row& row : rows) {
                ...
                for (const std::size_t index : {row.graph, row.fused, row.merged, row.loop}) {
        ...
        for (std::size_t r = 0UZ; r < kRingSizes.size(); ++r) {
            for (const Row& row : rows) {
                ...
                const double fused        = arms[row.fused].best;
                ...
                const double fusedStages  = fused - floorOf(graphFloor[r]);
                ...
                std::println("  N={} ring={:>5} : whole fused/graph {:5.2f} merged/graph {:5.2f} merged/loop {:5.2f} | stages fused/graph {} merged/graph {} merged/loop {}", //
                    row.nStages, sizeLabel(row.ring), fused / graph, merged / graph, merged / loop, ratio(fusedStages, graphStages), ratio(mergedStages, graphStages), ratio(mergedStages, loopStages));
            }
        }
        std::println("");
    };
