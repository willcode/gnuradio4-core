 ...
 * double-mapped ring, so a chain of N no-op blocks pays N ring round-trips. The arms
 * separate the candidate costs:
 *
 ...
 *   - scratch: the same arithmetic over one buffer with no graph, the lower bound on
 *     what the chain's arithmetic alone costs.
 *
...
// a chain of two processOne blocks, one processBulk block and one more processOne block
template<gr::scheduler::ExecutionPolicy policy>
[[nodiscard]] RunStats runMixedChain(std::size_t minBufferSize, std::size_t nSamples = kSamples) {
    using namespace boost::ut;
...
inline const boost::ut::suite<"chain cost"> _chain_cost = [] {
    using namespace boost::ut;
    ...
    for (const std::size_t nChain : kChainLengths) {
        ...
        for (const std::size_t chunk : kChunkSizes) {
            const std::string scratchName                = std::format("scratch     N={} chunk={:>6} (one buffer, no graph)", nChain, sizeLabel(chunk));
    ...
    for (const std::size_t ring : {kDefaultRing, kLargeRing}) {
        const std::string mixedName                = std::format("mixed       N=4 ring={:>7} fwd  singleThreaded", sizeLabel(ring));
        named(mixedName).repeat<kRepeat>(kSamples) = [ring] { expect(eq(runMixedChain<singleThreaded>(ring).nReceived, kSamples)); };
    }
};
