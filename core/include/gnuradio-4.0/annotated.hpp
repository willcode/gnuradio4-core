 ...
 * offset `t` must belong at output offset `t`. A block that shifts or drops sample positions —
 * `gr::blocks::basic::SampleDelay` shifts, `gr::blocks::basic::KeepOneInN` drops — writes its own `forwardTags()`
 * instead.
 */
