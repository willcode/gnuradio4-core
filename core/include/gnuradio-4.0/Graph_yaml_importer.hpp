...
inline LoadedBlocks loadGraphFromMap(PluginLoader& loader, gr::Graph& resultGraph, gr::property_map yaml, std::source_location location = std::source_location::current()) {
    ...
    for (const auto& blk : blks) {
        ...
        if (isSubgraph) {
            ...
            const auto                          instantiated  = pinnedVersion.has_value() ? loader.instantiatePinnedOrError(blockType, *pinnedVersion, recipeParameters) : loader.instantiateOrError(blockType, recipeParameters);
