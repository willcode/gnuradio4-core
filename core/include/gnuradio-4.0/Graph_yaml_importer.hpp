...
inline LoadedBlocks loadGraphFromMap(PluginLoader& loader, gr::Graph& resultGraph, gr::property_map yaml, OverrideUse* overrides, std::source_location location) {
    ...
    for (const auto& blk : blks) {
        ...
        if (isSubgraph) {
            ...
            // map, and every other key takes the ordinary settings path. A block that is not a
            // definition exports no parameter and keeps its whole map.
            ...
            const auto                          instantiated  = pinnedVersion.has_value() ? loader.instantiatePinnedOrError(blockType, *pinnedVersion, split.exported) : loader.instantiateOrError(blockType, split.exported);
            if (!instantiated.has_value()) {
                throw gr::exception(std::format("Unable to create block '{}' of type '{}': {}", blockName, blockType, instantiated.error().message));
            }
            auto currentBlock = *instantiated;
            if (!currentBlock) {
...
inline std::expected<std::shared_ptr<gr::BlockModel>, gr::Error> detail::instantiateBlockFromYamlDefinition(PluginLoader& loader, const detail::YamlDefinitionsLoader::Definition& def, const property_map& parameters) noexcept {
    try {
        ...
        auto read = recipeDeclarationsOf(definition);
        if (!read.has_value()) {
            return std::unexpected(read.error());
        }
        std::vector<recipe::ParameterDeclaration> declarations = std::move(*read);
